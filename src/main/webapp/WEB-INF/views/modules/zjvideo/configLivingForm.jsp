...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/zjvideo/config/">配置管理列表</a></li>
	...
	<form:form id="inputForm" modelAttribute="config" action="${ctx}/zjvideo/config/living/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
<%-- 		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">配置值：</label>
			<div class="controls">
				<select name="remark" id="itemName" style="width: 118px">  
	                 <option value="">请选择 </option>  
	                 <c:forEach items="${livings}" var="living">  
	                 	<c:if test="${living.id == config.remark}" > 
		                     <option selected="selected" value="${living.id}" >  
		                         ${living.livename}  
		                     </option> 
		                 </c:if>  
						 <c:if test="${living.id != config.remark}">
