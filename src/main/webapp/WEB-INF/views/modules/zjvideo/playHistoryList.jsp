...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/zjvideo/playHistory/">播放历史信息列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="playHistory" action="${ctx}/zjvideo/playHistory/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>用户姓名：</label>
				<form:input path="student.realname" htmlEscape="false" maxlength="50" class="input-medium"/>
			</li>
			<li><label>编号：</label>
			...
			<li>
				...
				<input id="beginDate"  name="addtime"  type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" style="width:163px;"
					value="<fmt:formatDate value="${playHistory.addtime}" pattern="yyyy-MM-dd"/>"
				...
				<input id="endDate" name="edittime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate" style="width:163px;"
					value="<fmt:formatDate value="${playHistory.edittime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
			</li>					
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="playHistory">
			<tr>
				<td>
					${(playHistory.type == 1) ? (playHistory.student.realname) : (playHistory.teacher.realname) }
				</td>	
				<td>
					${(playHistory.type == 1) ? (playHistory.student.studentno) : (playHistory.teacher.teacherno) }
				</td>
				<td>
					${fns:getDictLabel(playHistory.type, 'user_type', '')}
				</td>
				<td>
					${playHistory.program.name}
				</td>
				<td>
					${playHistory.programVolume.name}
				</td>
				<td>
