...
<body>
	...
	<table id="treeTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>机构名称</th><th>域名</th><th>站点logo</th><th>归属区域</th><th>机构识别码</th><th>机构编码</th><th>机构类型</th><th>备注</th><shiro:hasPermission name="sys:office:edit"><th>操作</th></shiro:hasPermission></tr></thead>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<td>{{row.domain}}</td>
			<td><img src="{{row.logo}}" height="60" width="80"></img></td>
			<td>{{row.area.name}}</td>
			<td>company_{{row.id}}</td>
			<td>{{row.code}}</td>
			<td>{{dict.type}}</td>
			<td>{{row.remarks}}</td>
